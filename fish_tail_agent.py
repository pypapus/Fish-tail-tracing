...
class FishTailAgent:
    ...
    def _estimate_notochord_endpoint(
        ...
        """Trace a potentially curved notochord and detect its distal endpoint.

        V4.1 keeps the researcher-calibrated endpoint prior, but no longer assumes
        that the terminal notochord is horizontal. It traces a smooth centerline
        from the clearly visible proximal notochord toward the expected endpoint,
        allowing gradual vertical bending.

        Endpoint evidence combines:
        - the researcher-derived expected endpoint location,
        - weakening/termination of the traced notochord,
        - the proximal edge (BEGINNING) of the dark distal stain.

        The final ROI cutoff remains vertical, as required.
        """
        ...
        # Researcher-calibrated endpoint prior from the 9 reference ROIs.
        ...
        # Notochord borders are predominantly elongated along X, but the response
        # remains useful when the terminal segment bends.
        horizontal = np.maximum(np.abs(gy) - 0.40 * np.abs(gx), 0.0)
        horizontal = cv2.GaussianBlur(horizontal, (0, 0), 1.0)

        # Dark-stain evidence. We use its proximal onset, not its distal end.
        ...
        dark = (
            0.58 * (255.0 - blur.astype(np.float32))
            + 0.42 * blackhat.astype(np.float32)
        )

        # --------------------------------------------------------------
        # 1. Find a reliable proximal seed row.
        # --------------------------------------------------------------
        seed_x0 = int(np.clip(x + 0.06 * bw, x, x + bw - 2))
        seed_x1 = int(np.clip(x + 0.30 * bw, seed_x0 + 1, x + bw - 1))
        seed_y0 = int(max(y, prior_y - 0.18 * bh))
        seed_y1 = int(min(y + bh, prior_y + 0.18 * bh))

        seed_band = horizontal[seed_y0:seed_y1, seed_x0:seed_x1]
        if seed_band.size:
            row_score = np.percentile(seed_band, 82, axis=1).astype(np.float32)
            row_score = cv2.GaussianBlur(
                row_score.reshape(-1, 1),
                (0, 0),
                sigmaX=0, sigmaY=max(1.0, 0.018 * bh),
            ).reshape(-1)
            seed_y = float(seed_y0 + int(np.argmax(row_score)))
        else:
            seed_y = prior_y

        # --------------------------------------------------------------
        # 2. Trace the centerline while allowing a gradual bend.
        # --------------------------------------------------------------
        trace_start_x = seed_x0
        trace_end_x = int(min(x + bw - 2, prior_x + 0.12 * bw))
        step = max(2, int(round(bw / 180.0)))
        trace_xs = list(range(trace_start_x, trace_end_x + 1, step))

        trace_points = []
        trace_strengths = []
        current_y = float(seed_y)

        # Maximum vertical motion per step prevents jumping to unrelated stains,
        # but still permits the curved terminal notochord seen in Ctrl 4.
        max_jump = max(3, int(round(0.018 * bh)))
        search_half = max(10, int(round(0.055 * bh)))

        for xx in trace_xs:
            yy0 = int(max(y, round(current_y - search_half)))
            yy1 = int(min(y + bh, round(current_y + search_half + 1)))
            if yy1 <= yy0:
                continue

            local = horizontal[yy0:yy1, xx].astype(np.float32)
            if local.size == 0:
                continue

            # Smooth local response and add a continuity preference centered on
            # the previous traced location.
            local = cv2.GaussianBlur(
                local.reshape(-1, 1), (0, 0), sigmaX=0, sigmaY=1.3
            ).reshape(-1)

            yy = np.arange(yy0, yy1, dtype=np.float32)
            continuity = np.exp(
                -0.5 * ((yy - current_y) / max(4.0, 0.025 * bh)) ** 2
            ).astype(np.float32)

            response_scale = max(float(np.percentile(local, 90)), 1e-6)
            combined = 0.72 * (local / response_scale) + 0.28 * continuity
            best_local = int(np.argmax(combined))
            candidate_y = float(yy0 + best_local)

            delta = float(np.clip(candidate_y - current_y, -max_jump, max_jump))
            current_y = current_y + delta

            trace_points.append([float(xx), float(current_y)])
            trace_strengths.append(float(local[best_local]))

        if len(trace_points) < 6:
            trace_points = [
                [float(trace_start_x), float(seed_y)],
                [float(prior_x), float(prior_y)],
            ]
            trace_strengths = [0.0, 0.0]

        # Smooth the traced Y coordinates without forcing them to be straight.
        pts = np.asarray(trace_points, dtype=np.float32)
        if len(pts) >= 7:
            ys = pts[:, 1].copy()
            k = min(11, len(ys) if len(ys) % 2 == 1 else len(ys) - 1)
            if k >= 5:
                ys_smooth = cv2.GaussianBlur(
                    ys.reshape(-1, 1), (1, k), sigmaX=0, sigmaY=max(1.0, k / 4.0)
                ).reshape(-1)
                pts[:, 1] = ys_smooth
        trace_points = pts.tolist()

        # --------------------------------------------------------------
        # 3. Score endpoint positions along the curved trace.
        # --------------------------------------------------------------
        xs_arr = np.asarray([p[0] for p in trace_points], dtype=np.float32)
        ys_arr = np.asarray([p[1] for p in trace_points], dtype=np.float32)

        line_profile = []
        stain_profile = []
        band_half = max(8, int(round(0.045 * bh)))

        for xx_f, yy_f in zip(xs_arr, ys_arr):
            xx = int(np.clip(round(xx_f), 0, w - 1))
            yy0 = int(max(y, round(yy_f - band_half)))
            yy1 = int(min(y + bh, round(yy_f + band_half + 1)))
            if yy1 <= yy0:
                line_profile.append(0.0)
                stain_profile.append(0.0)
                continue
            line_profile.append(
                float(np.percentile(horizontal[yy0:yy1, xx], 88))
            )
            stain_profile.append(
                float(np.percentile(dark[yy0:yy1, xx], 88))
            )

        line_profile = np.asarray(line_profile, dtype=np.float32)
        stain_profile = np.asarray(stain_profile, dtype=np.float32)

        if len(line_profile) >= 5:
            line_s = cv2.GaussianBlur(
                line_profile.reshape(1, -1), (0, 0), sigmaX=1.2
            ).reshape(-1)
            stain_s = cv2.GaussianBlur(
                stain_profile.reshape(1, -1), (0, 0), sigmaX=1.2
            ).reshape(-1)
            dline = np.gradient(line_s)
            dstain = np.gradient(stain_s)
            line_scale = max(float(np.percentile(np.abs(dline), 90)), 1e-6)
            stain_scale = max(float(np.percentile(np.abs(dstain), 90)), 1e-6)
        else:
            dline = np.zeros_like(line_profile)
            dstain = np.zeros_like(stain_profile)
            line_scale = stain_scale = 1.0

        best_idx = int(np.argmin(np.abs(xs_arr - prior_x)))
        best_score = -1.0
        best_stain = 0.0
        best_line = 0.0
        best_prior = 0.0

        sigma_x = max(10.0, 0.055 * bw)
        sigma_y = max(10.0, 0.080 * bh)

        for i in range(len(xs_arr)):
            # Keep endpoint search near the calibrated anatomical region.
            px_score = np.exp(-0.5 * ((xs_arr[i] - prior_x) / sigma_x) ** 2)
            py_score = np.exp(-0.5 * ((ys_arr[i] - prior_y) / sigma_y) ** 2)
            prior_score = float(0.70 * px_score + 0.30 * py_score)

            stain_onset = float(np.clip(dstain[i] / stain_scale, 0.0, 1.0))
            line_term = float(np.clip((-dline[i]) / line_scale, 0.0, 1.0))
        ...
        endpoint_x = int(round(xs_arr[best_idx]))
        endpoint_y = float(ys_arr[best_idx])

        # Find actual MobileSAM mask span at/near endpoint for vertical cutoff.
        nearby = []
        ...
        if nearby:
            ...
            cutoff_top = int(yy.min())
            cutoff_bottom = int(yy.max())
        else:
            cutoff_top = int(y)
            cutoff_bottom = int(y + bh - 1)

        # Quantify terminal bend for diagnostics.
        terminal_n = min(8, len(trace_points))
        if terminal_n >= 3:
            tx = xs_arr[-terminal_n:]
            ty = ys_arr[-terminal_n:]
            coef = np.polyfit(tx, ty, 1)
            terminal_slope = float(coef[0])
            terminal_angle = float(np.degrees(np.arctan(terminal_slope)))
        else:
            terminal_slope = 0.0
            terminal_angle = 0.0
        ...
        # Trim displayed trace at the chosen endpoint.
        final_trace = [
            [float(px), float(py)]
            for px, py in trace_points
            if px <= endpoint_x
        ]
        if not final_trace or final_trace[-1][0] < endpoint_x:
            final_trace.append([float(endpoint_x), float(endpoint_y)])
        ...
        return {
            ...
            "method": "curved_notochord_trace_plus_stain_onset",
            "axis_slope_pixels_per_pixel": terminal_slope,
            "terminal_angle_degrees": terminal_angle,
            "trace_points": final_trace,
            ...
            "curved_trace_points": int(len(final_trace)),
    ...
    def process_file(self, filepath: Path, output_dir: Optional[Path] = None) -> Dict[str, Any]:
        ...
        self._log(f"Processing: {filepath.name} | build=V4_1_MOBILESAM_CURVED_NOTOCHORD_2026-09-19")
        ...
        record = {
            "file_metadata": {
                ...
                "agent_build": "V4_1_MOBILESAM_CURVED_NOTOCHORD_2026-09-19",
