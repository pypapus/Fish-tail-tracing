...
st.info(
    ...
    "the detected end of the notochord. The AI model first segments the outer tail independently, then a separate landmark step traces "
...
st.subheader("AI tail model")
tail_model_choice = st.selectbox(
    "Segmentation backend",
    options=[
        "Auto — MobileSAM first, SAM2.1-tiny fallback",
        "MobileSAM only",
        "SAM2.1-tiny only",
    ],
    index=0,
    help=(
        "Auto is recommended. MobileSAM is tried first because it performed well on the original tail image. "
        "SAM2.1-tiny is only loaded if MobileSAM cannot produce a plausible tail mask."
    ),
)
tail_model_map = {
    "Auto — MobileSAM first, SAM2.1-tiny fallback": "auto",
    "MobileSAM only": "mobilesam",
    "SAM2.1-tiny only": "sam2.1_tiny",
}
tail_model = tail_model_map[tail_model_choice]

st.caption(
    "V3 architecture: the AI model owns the outer tail boundary. "
    "Notochord endpoint detection is a separate anatomical-landmark step and cannot reshape the distal tail contour."
)
