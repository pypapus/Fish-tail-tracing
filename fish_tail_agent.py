...
class FishTailAgent:
    ...
    def assess_quality(
        ...
        # Final QA deliberately limits the influence of shape/geometry alone.
        ...
        quality_score = int(round(np.clip(
            (0.30 * (focus_score / 100.0) + 0.70 * segmentation_confidence) * 100,
            0, 100
        )))
        if segmentation_confidence < 0.55:
            quality_score = min(quality_score, 49)

        model_selected = segmentation.get("method") == "mobilesam_final"
        ...
        if segmentation_confidence < 0.65 or plausibility == "fail":
            ...
            recommendation = (
                f"Boundary appears usable but image-edge clipping detected ({edges}). "
                "Measurements describe the visible portion only; validate ROI in Fiji before analysis."
            )
        elif model_selected and segmentation_confidence >= 0.80 and plausibility == "pass" and quality_score >= 75:
            flag = "good"
            recommendation = "Proceed after routine visual QA in Fiji"
        else:
            flag = "review"
            recommendation = "Review boundary PNG/ROI in Fiji before using measurements"
    ...
    def _save_boundary_overlay_preview(
        self, original: np.ndarray, contour: np.ndarray, out_path: Path
    ) -> str:
        original_u8 = self._normalize_to_uint8(original)
    ...
    def process_array(
        ...
        segmentation["boundary_evidence_score"] = self._boundary_evidence_score(
            self._normalize_to_uint8(bf), segmentation["contour"]
        )
        segmentation["edge_clipping"] = self._edge_clipping_status(
            segmentation["contour"], self._normalize_to_uint8(bf).shape
        )
    ...
    def process_file(self, filepath: Path, output_dir: Optional[Path] = None) -> Dict[str, Any]:
        ...
        self._log(f"Processing: {filepath.name} | build=FINAL_DASHBOARD_MOBILESAM_2026-09-13")
        ...
        if self.config.save_png:
            try:
                ...
                self._save_boundary_overlay_preview(
                    bf, processed["contour"], overlay_preview_path
                )
            except Exception as exc:
                self._log(f"User preview generation failed for {filepath.name}: {exc}")

            if processed["segmentation"].get("method") == "mobilesam_final":
                try:
                    self._save_raw_mask(processed["binary"], raw_mask_path)
                except Exception as exc:
                    self._log(f"Raw model mask generation failed for {filepath.name}: {exc}")
            try:
        ...
        record = {
            "file_metadata": {
                ...
                "agent_build": "FINAL_DASHBOARD_MOBILESAM_2026-09-13",
            ...
            "processing_notes": {
                ...
                "model_status": "model_selected" if seg.get("method") == "mobilesam_final" else "model_attempted_but_fallback_selected",
    ...
    def export_summary_csv(self, output_dir: Path) -> Optional[Path]:
        ...
        for extra in ("Pixel_Size_um", "Calibration_Source", "Area_um2", "Perimeter_um", "Width_um", "Height_um", "Centroid_X_um", "Centroid_Y_um"):
