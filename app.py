...
st.title("🐟 Fish Tail Boundary Agent — Test 3 MobileSAM")
st.success("Build: TEST3_MOBILESAM_2026-09-09 — zero-shot MobileSAM model-assisted segmentation")
st.write(
    "Upload Zeiss `.czi` microscopy files. This experimental build uses MobileSAM to propose the full translucent fin mask from a generous anatomical prompt box. The agent will detect the brightfield "
...
if run:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if csv_path and Path(csv_path).exists():
            ...
            method_cols = [c for c in ["Threshold_Method", "Segmentation_Method", "Method"] if c in df.columns]
            if method_cols:
                st.info("Segmentation method used: " + str(df.iloc[0][method_cols[0]]))
        ...
        model_images = sorted(analyzed_dir.glob("*_model_test3.png"))
        if model_images:
            st.subheader("Test 3 — MobileSAM Model Diagnostic")
            st.caption(
                "Panel 1 shows the old method only as a rough locator and the expanded prompt box. "
                "Panel 2 shows MobileSAM's proposed mask. Panel 3 shows the boundary actually selected for measurement."
            )
            for img in model_images[:12]:
                st.image(str(img), caption=img.name, use_container_width=True)
