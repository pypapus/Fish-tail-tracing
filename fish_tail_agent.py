...
class FishTailAgent:
    ...
    def _estimate_notochord_endpoint(
        ...
        Biological rule used by this build:
        - identify the long, approximately horizontal notochord signal;
        - locate the point where that coherent horizontal signal terminates;
        - use a VERTICAL cutoff through that x-coordinate;
        - retain only fin tissue distal to the cutoff.

        The detector is deliberately conservative. If line evidence is weak, it
        returns a best-effort estimate with reduced confidence so the final ROI is
        ...
        # Central band: the notochord is expected near the body/fin midline.
        ...
        # Horizontal anatomical structures produce a strong vertical intensity
        # gradient and relatively weaker horizontal gradient.
        gx = cv2.Sobel(blur, cv2.CV_32F, 1, 0, ksize=3)
        gy = cv2.Sobel(blur, cv2.CV_32F, 0, 1, ksize=3)
        ...
        # Robust per-column response. The 88th percentile is sensitive to the
        # paired notochord edges without being dominated by most background pixels.
        profile = np.percentile(roi_evidence, 88, axis=0).astype(np.float32)
        sigma = max(1.5, 0.012 * bw)
        profile_s = cv2.GaussianBlur(
            profile.reshape(1, -1), (0, 0), sigmaX=sigma
        ).reshape(-1)
        ...
        # When the broad SAM mask reaches only one image edge, that edge is a
        # strong clue for the body/proximal side. Otherwise use horizontal-line
        # evidence: the notochord/body side should have stronger sustained signal.
        ...
        endpoint_rel = None
        detection_method = "horizontal_line_drop"
        ...
        # If there is no clean threshold crossing, use the strongest sustained
        # negative slope as a best-effort estimate.
        if endpoint_rel is None and hi - lo > 3:
            ...
            detection_method = "horizontal_line_drop_derivative"

        # Last-resort geometric estimate. This is intentionally labeled and
        # confidence-limited so it can never be mistaken for a strong detection.
        ...
        # Estimate a simple notochord centerline from the centroid of horizontal
        # evidence in each proximal column, then robustly fit a shallow line.
        if proximal_side == "left":
            sample_start = x + max(2, int(0.04 * bw))
            sample_end = max(sample_start + 1, endpoint_x - max(2, int(0.02 * bw)))
            x_samples = range(sample_start, sample_end, max(1, bw // 120))
        else:
            sample_start = x + bw - 1 - max(2, int(0.04 * bw))
            sample_end = min(sample_start - 1, endpoint_x + max(2, int(0.02 * bw)))
            x_samples = range(sample_start, sample_end, -max(1, bw // 120))
        ...
        if len(fit_x) >= 8:
            ...
            endpoint_y = float(y + 0.5 * bh)
            proximal_x = float(x if proximal_side == "left" else x + bw - 1)
            proximal_y = endpoint_y
            axis_slope = 0.0

        endpoint_y = float(np.clip(endpoint_y, y0, y1 - 1))
        proximal_y = float(np.clip(proximal_y, y0, y1 - 1))

        # Locate the actual mask intersection for the vertical cutoff. Search a
        # few columns around the estimated endpoint and pick the one with the most
        # fin pixels; this avoids gaps from single-pixel mask irregularities.
        best_col = endpoint_x
        best_ys = np.flatnonzero(mask[:, endpoint_x]) if 0 <= endpoint_x < w else np.array([], dtype=int)
        for xx in range(max(0, endpoint_x - 3), min(w, endpoint_x + 4)):
            ys_here = np.flatnonzero(mask[:, xx])
            if len(ys_here) > len(best_ys):
                best_col, best_ys = xx, ys_here

        endpoint_x = int(best_col)

        if len(best_ys):
            # Split into contiguous vertical runs; select the run containing the
            # estimated notochord y, otherwise the longest run.
        ...
        # Confidence combines: signal drop, side/orientation certainty, signal
        # separation, and a biologically plausible landmark position.
        if baseline > 1e-6:
            post_start = min(len(directional), endpoint_rel + sustain)
            post_end = min(len(directional), post_start + max(8, int(0.16 * bw)))
            post = float(np.median(directional[post_start:post_end])) if post_end > post_start else baseline
            drop_score = float(np.clip(1.0 - post / max(baseline, 1e-6), 0.0, 1.0))
        else:
            drop_score = 0.0
        ...
        if 0.35 <= rel <= 0.85:
            ...
            location_score = float(np.clip(1.0 - abs(rel - 0.60) / 0.60, 0.0, 1.0))

        confidence = float(np.clip(
            0.45 * drop_score
            + 0.20 * orientation_conf
            + 0.20 * separation_score
            + 0.15 * location_score,
            0.0, 1.0
        ))

        if detection_method == "horizontal_line_drop_derivative":
            confidence = min(confidence * 0.85, 0.68)
        elif detection_method == "geometric_fallback":
        ...
        return {
            ...
            "review_required": bool(confidence < 0.60 or detection_method == "geometric_fallback"),
        }
    ...
    def process_file(self, filepath: Path, output_dir: Optional[Path] = None) -> Dict[str, Any]:
        ...
        self._log(f"Processing: {filepath.name} | build=FINAL_NOTOCHORD_2026-09-16")
        ...
        record = {
            "file_metadata": {
                ...
                "agent_build": "FINAL_NOTOCHORD_2026-09-16",
            ...
            "processing_notes": {
                ...
                "roi_definition": "outer fin distal to vertical line through detected notochord endpoint",
            },
        }
