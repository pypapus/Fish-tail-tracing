...
class AgentConfig:
    ...
    save_roi: bool = True
    verbose: bool = True
    tail_model: str = "auto"  # auto | mobilesam | sam2.1_tiny
...
class FishTailAgent:
    ...
    def _estimate_notochord_endpoint(
        ...
        """Estimate the distal endpoint of the notochord.

        Updated biological rule:
        - segment the broad outer fin envelope;
        - identify the long, approximately horizontal notochord tube;
        - use BOTH the line termination and the dark stain/clumped tissue near the
          distal notochord end as guides;
        - place a VERTICAL cutoff through that endpoint;
        - retain only fin tissue distal to the cutoff.

        The detector is conservative. If line/stain evidence is weak or conflicted,
        it still returns a best-effort estimate but reduces confidence so the image
        ...
        # The notochord is expected near the central body/fin band.
        ...
        # Horizontal anatomical structures produce strong vertical intensity
        # gradients and comparatively weaker horizontal gradients.
        gx = cv2.Sobel(blur, cv2.CV_32F, 1, 0, ksize=3)
        gy = cv2.Sobel(blur, cv2.CV_32F, 0, 1, ksize=3)
        horizontal_evidence = np.maximum(np.abs(gy) - 0.65 * np.abs(gx), 0.0)

        roi_evidence = horizontal_evidence[y0:y1, x:x + bw]
        if roi_evidence.size == 0:
            roi_evidence = horizontal_evidence[y:y + bh, x:x + bw]

        # Column-wise notochord-edge evidence.
        profile = np.percentile(roi_evidence, 88, axis=0).astype(np.float32)
        sigma = max(1.5, 0.012 * bw)
        profile_s = cv2.GaussianBlur(profile.reshape(1, -1), (0, 0), sigmaX=sigma).reshape(-1)

        side_n = max(8, int(round(0.28 * bw)))
        side_n = min(side_n, max(1, bw // 2))
        left_score = float(np.median(profile_s[:side_n]))
        right_score = float(np.median(profile_s[-side_n:]))

        edge_margin = max(3, int(round(0.005 * w)))
        touches_left = x <= edge_margin
        touches_right = (x + bw - 1) >= (w - 1 - edge_margin)

        # Determine which side is proximal/body side.
        if touches_left and not touches_right:
            ...
            orientation_conf = float(np.clip(abs(left_score - right_score) / denom, 0.0, 1.0))

        directional = profile_s.copy() if proximal_side == "left" else profile_s[::-1].copy()
        base_n = max(10, int(round(0.30 * bw)))
        base_n = min(base_n, len(directional))
        baseline = float(np.percentile(directional[:base_n], 65)) if base_n else 0.0

        # ----- Step 1: preliminary endpoint from line drop -----
        lo = int(round(0.25 * bw))
        hi = int(round(0.92 * bw))
        lo = int(np.clip(lo, 1, max(1, bw - 2)))
        hi = int(np.clip(hi, lo + 1, bw))
        sustain = max(6, int(round(0.045 * bw)))
        threshold = 0.52 * baseline

        endpoint_rel = None
        detection_method = "line_drop_plus_stain"

        if baseline > 1e-6:
            for i in range(lo, max(lo + 1, hi - sustain)):
                window = directional[i:i + sustain]
                if (
                    directional[i] < threshold
                    and len(window) > 0
                    and float(np.mean(window < (0.65 * baseline))) >= 0.80
                ):
                    endpoint_rel = int(i)
                    break

        if endpoint_rel is None and hi - lo > 3:
            derivative = np.gradient(directional)
            local = derivative[lo:hi]
            endpoint_rel = int(lo + np.argmin(local))
            detection_method = "line_derivative_plus_stain"

        if endpoint_rel is None:
            endpoint_rel = int(round(0.65 * (bw - 1)))
            detection_method = "geometric_plus_stain_fallback"

        endpoint_rel = int(np.clip(endpoint_rel, 1, bw - 2))

        # ----- Step 2: fit approximate notochord centerline -----
        if proximal_side == "left":
            sample_start = x + max(2, int(0.04 * bw))
            # fit well into the proximal side so the endpoint estimate does not
            # control the line fit.
            sample_end = min(x + bw - 2, x + max(int(0.60 * bw), endpoint_rel))
            x_samples = range(sample_start, max(sample_start + 1, sample_end), max(1, bw // 120))
        else:
            sample_start = x + bw - 1 - max(2, int(0.04 * bw))
            sample_end = max(x + 1, x + bw - 1 - max(int(0.60 * bw), endpoint_rel))
            x_samples = range(sample_start, min(sample_start - 1, sample_end), -max(1, bw // 120))
        ...
        for xx in x_samples:
            ...
            if vals.size == 0 or float(vals.max()) <= 1e-6:
                continue
            local_thr = float(np.percentile(vals, 85))
            weights = np.where(vals >= local_thr, vals, 0.0)
        ...
        if len(fit_x) >= 8:
            ...
            axis_slope = float(coef[0])
            proximal_x = float(x if proximal_side == "left" else x + bw - 1)
            proximal_y = float(np.polyval(coef, proximal_x))
        else:
            coef = np.array([0.0, y + 0.5 * bh], dtype=float)
            axis_slope = 0.0
            proximal_x = float(x if proximal_side == "left" else x + bw - 1)
            proximal_y = float(y + 0.5 * bh)

        proximal_y = float(np.clip(proximal_y, y0, y1 - 1))

        # ----- Step 3: stain cue near the distal notochord end -----
        # Build a dark-structure map emphasizing the stain/clumped tissue.
        ...
        inv = 255.0 - blur.astype(np.float32)
        dark_map = 0.55 * inv + 0.45 * blackhat.astype(np.float32)

        directional_dark = np.zeros(bw, dtype=np.float32)
        band_half = max(8, int(round(0.06 * bh)))
        for i in range(bw):
            xx = x + i if proximal_side == "left" else x + (bw - 1 - i)
            yc = float(np.polyval(coef, xx))
            yc = float(np.clip(yc, y0, y1 - 1))
            ys0 = int(max(y0, round(yc - band_half)))
            ys1 = int(min(y1, round(yc + band_half)))
            if ys1 <= ys0 or not (0 <= xx < w):
                continue
            region = dark_map[ys0:ys1, xx]
            if region.size == 0:
                continue
            # Use upper-tail intensity of the dark map to emphasize punctate dark stain.
            directional_dark[i] = float(np.percentile(region, 88))

        directional_dark_s = cv2.GaussianBlur(
            directional_dark.reshape(1, -1), (0, 0), sigmaX=max(1.5, 0.010 * bw)
        ).reshape(-1)

        stain_base_n = max(10, int(round(0.25 * bw)))
        stain_base_n = min(stain_base_n, len(directional_dark_s))
        stain_baseline = float(np.percentile(directional_dark_s[:stain_base_n], 55)) if stain_base_n else 0.0

        # Search primarily at/after the preliminary line-drop estimate, because the
        # stain is expected near the distal termination rather than well proximal.
        search_lo = int(np.clip(endpoint_rel - max(4, int(0.03 * bw)), 1, bw - 2))
        search_hi = int(np.clip(int(round(0.96 * bw)), search_lo + 1, bw))
        deriv = np.gradient(directional)

        best_idx = endpoint_rel
        best_score = -1.0
        best_stain_component = 0.0
        best_line_component = 0.0
        best_deriv_component = 0.0

        line_scale = max(
            float(np.percentile(np.abs(deriv[lo:hi]), 90)) if hi > lo else 0.0,
            1e-6,
        )
        stain_range = max(
            float(np.percentile(directional_dark_s[search_lo:search_hi], 92) - stain_baseline)
            if search_hi > search_lo else 0.0,
            1e-6,
        )

        for i in range(search_lo, search_hi):
            # Stronger when the line-evidence has dropped relative to proximal baseline.
            line_component = float(np.clip((baseline - directional[i]) / max(baseline, 1e-6), 0.0, 1.0))
            # Stronger when a dark stain/clump is present near the centerline.
            stain_component = float(np.clip((directional_dark_s[i] - stain_baseline) / stain_range, 0.0, 1.0))
            # Stronger where horizontal-line evidence is actively falling.
            deriv_component = float(np.clip((-deriv[i]) / line_scale, 0.0, 1.0))
            # Mild biological prior favoring the middle-distal region.
            rel = i / max(float(bw - 1), 1.0)
            location_prior = float(np.clip(1.0 - abs(rel - 0.68) / 0.38, 0.0, 1.0))

            combined = (
                0.42 * stain_component
                + 0.28 * line_component
                + 0.15 * deriv_component
                + 0.15 * location_prior
            )
            if combined > best_score:
                best_score = combined
                best_idx = i
                best_stain_component = stain_component
                best_line_component = line_component
                best_deriv_component = deriv_component

        # If a plausible stain-supported candidate exists later than the initial
        # line drop, prefer it. This specifically addresses the earlier failure
        # mode where the detector stopped too proximally.
        endpoint_rel = int(np.clip(best_idx, 1, bw - 2))

        if proximal_side == "left":
            endpoint_x = int(x + endpoint_rel)
        else:
            endpoint_x = int(x + (bw - 1 - endpoint_rel))

        endpoint_y = float(np.polyval(coef, endpoint_x))
        endpoint_y = float(np.clip(endpoint_y, y0, y1 - 1))

        # Search nearby columns and choose the one with the strongest stain support.
        best_col = endpoint_x
        best_col_score = -1.0
        best_ys = np.flatnonzero(mask[:, endpoint_x]) if 0 <= endpoint_x < w else np.array([], dtype=int)
        for xx in range(max(0, endpoint_x - 4), min(w, endpoint_x + 5)):
            ys_here = np.flatnonzero(mask[:, xx])
            if len(ys_here) == 0:
                continue
            yc = float(np.polyval(coef, xx))
            yc = float(np.clip(yc, y0, y1 - 1))
            ys0 = int(max(y0, round(yc - band_half)))
            ys1 = int(min(y1, round(yc + band_half)))
            stain_here = float(np.percentile(dark_map[ys0:ys1, xx], 88)) if ys1 > ys0 else 0.0
            score_here = stain_here + 0.20 * len(ys_here)
            if score_here > best_col_score:
                best_col_score = score_here
                best_col = xx
                best_ys = ys_here

        endpoint_x = int(best_col)
        endpoint_y = float(np.polyval(coef, endpoint_x))
        endpoint_y = float(np.clip(endpoint_y, y0, y1 - 1))

        if len(best_ys):
            splits = np.where(np.diff(best_ys) > 1)[0] + 1
            runs = np.split(best_ys, splits)
            containing = [r for r in runs if len(r) and r[0] <= endpoint_y <= r[-1]]
            chosen = containing[0] if containing else max(runs, key=len)
            cutoff_top = int(chosen[0])
            cutoff_bottom = int(chosen[-1])
        else:
            cutoff_top = int(y)
            cutoff_bottom = int(y + bh - 1)

        # Confidence: combine line evidence, stain cue, side certainty, and
        # reasonable endpoint location.
        distal_side_score = right_score if proximal_side == "left" else left_score
        proximal_side_score = left_score if proximal_side == "left" else right_score
        separation_score = float(np.clip(
            (proximal_side_score - distal_side_score) / max(proximal_side_score, 1e-6),
            0.0, 1.0
        ))

        rel = endpoint_rel / max(float(bw - 1), 1.0)
        if 0.40 <= rel <= 0.88:
            location_score = 1.0
        else:
            location_score = float(np.clip(1.0 - abs(rel - 0.68) / 0.68, 0.0, 1.0))

        confidence = float(np.clip(
            0.30 * best_line_component
            + 0.32 * best_stain_component
            + 0.08 * best_deriv_component
        ...
        if detection_method == "line_derivative_plus_stain":
            confidence = min(confidence * 0.90, 0.74)
        elif detection_method == "geometric_plus_stain_fallback":
        ...
        return {
            ...
            "axis_slope_pixels_per_pixel": axis_slope,
            "trace_points": trace_points,
            "profile_baseline": float(baseline),
            "profile_left_score": float(left_score),
            "profile_right_score": float(right_score),
            "line_drop_component": float(best_line_component),
            "stain_component": float(best_stain_component),
            "derivative_component": float(best_deriv_component),
            "orientation_confidence": float(orientation_conf),
            "location_fraction_from_proximal": float(rel),
            "stain_guided": True,
            "review_required": bool(confidence < 0.62 or detection_method == "geometric_plus_stain_fallback"),
        }
    ...
    def _direct_tail_prompt_boxes(self, original: np.ndarray) -> List[List[int]]:
        """Generate model prompts without requiring classical segmentation.

        The old pipeline failed five images before MobileSAM ever ran. This version
        always gives the segmentation model a chance first. We propose several broad
        boxes covering both possible tail orientations and let model-mask scoring
        decide which one is biologically plausible.
        """
        img = self._normalize_to_uint8(original)
        h, w = img.shape[:2]

        # Central vertical range covers the fish while excluding only small top/bottom margins.
        y0 = int(round(0.06 * h))
        y1 = int(round(0.94 * h))

        boxes = [
            [0, y0, int(round(0.62 * w)), y1],                         # left-side tail candidate
            [int(round(0.38 * w)), y0, w - 1, y1],                    # right-side tail candidate
            [0, int(round(0.14 * h)), int(round(0.72 * w)), int(round(0.86 * h))],
            [int(round(0.28 * w)), int(round(0.14 * h)), w - 1, int(round(0.86 * h))],
            [int(round(0.12 * w)), int(round(0.10 * h)), int(round(0.88 * w)), int(round(0.90 * h))],
        ]
    ...
    def _tail_boundary_score(
        ...
        """Score model masks using researcher-informed tail geometry without reshaping them.

        Important: the researcher samples are used here only to SELECT among model
        masks. They do not overwrite or warp the MobileSAM/SAM2 outer contour.
        """
        img = self._normalize_to_uint8(original)
        h, w = img.shape[:2]
        area = float(cv2.contourArea(contour))
        ...
        # Manual researcher ROIs cluster around width/height ~0.85.
        aspect_score = float(np.exp(-0.5 * ((aspect - 0.85) / 0.35) ** 2))

        # Tail regions should occupy a meaningful but not whole-image footprint.
        area_score = float(np.clip(1.0 - abs(af - 0.18) / 0.22, 0.0, 1.0))
        width_score = float(np.clip(1.0 - abs(wf - 0.42) / 0.35, 0.0, 1.0))
        height_score = float(np.clip(1.0 - abs(hf - 0.42) / 0.35, 0.0, 1.0))
        solidity_score = float(np.clip((solidity - 0.45) / 0.50, 0.0, 1.0))

        # Independent image-edge support: weak translucent boundaries are allowed,
        # but masks whose perimeter has some visible evidence are preferred.
        ...
        edge_score = 0.5 if vals.size == 0 or ref <= 1e-6 else float(
            np.clip(np.median(vals) / ref, 0.0, 1.0)
        )

        # Penalize obvious whole-fish / tiny-object masks.
        penalty = 0.0
        if wf > 0.82 or hf > 0.82 or af > 0.58:
            penalty += 0.45
        if wf < 0.16 or hf < 0.14 or af < 0.008:
            penalty += 0.55
        ...
        return float(np.clip(score, 0.0, 1.0)), {
            ...
            "boundary_evidence_score": float(edge_score),
        }
    ...
    def _run_prompted_sam_model(
        ...
        try:
            ...
            success = (
                best["score"] >= 0.44
                and best["area_fraction"] >= 0.010
                and best["bbox_width_fraction"] >= 0.16
                and best["bbox_height_fraction"] >= 0.14
                and best["bbox_width_fraction"] <= 0.82
                and best["bbox_height_fraction"] <= 0.82
            )
    ...
    def _direct_tail_model_candidate(self, original: np.ndarray) -> Dict[str, Any]:
        """Auto backend: MobileSAM first; SAM2.1-tiny only if needed.

        This preserves the model that already worked well on the user's first fish,
        while providing a stronger fallback without blocking model inference behind
        classical segmentation.
        """
        mode = str(getattr(self.config, "tail_model", "auto")).strip().lower()

        if mode == "mobilesam":
            return self._run_prompted_sam_model(original, "mobilesam")

        if mode in ("sam2.1_tiny", "sam2", "sam2.1"):
            return self._run_prompted_sam_model(original, "sam2.1_tiny")

        mobile = self._run_prompted_sam_model(original, "mobilesam")
        if mobile.get("success"):
            mobile["auto_fallback_used"] = False
            return mobile

        sam2 = self._run_prompted_sam_model(original, "sam2.1_tiny")
        sam2["auto_fallback_used"] = True
        sam2["mobile_attempt_error"] = mobile.get("error")
        return sam2
    ...
    def process_file(self, filepath: Path, output_dir: Optional[Path] = None) -> Dict[str, Any]:
        ...
        self._log(f"Processing: {filepath.name} | build=V3_DUALMODEL_SEPARATE_NOTOCHORD_2026-09-17")
        ...
        record = {
            "file_metadata": {
                ...
                "agent_build": "V3_DUALMODEL_SEPARATE_NOTOCHORD_2026-09-17",
