...
st.caption(
    "Researcher-informed caudal-fin morphometry for Zeiss CZI microscopy images"
)
                ...
                with st.expander(f"{'✅' if flag.lower()=='good' else '⚠️' if flag.lower()=='review' else '❌'} {filename}", expanded=len(df) == 1):
                    ...
                    with tabs[2]:
                        if overlay_path.exists():
                            st.image(
                                str(overlay_path),
                                caption="Final researcher-informed ROI preserving the MobileSAM outer tail boundary",
                                use_container_width=True,
                            )
