...
class FishTailAgent:
    ...
    def _save_boundary_test2_png(self, original: np.ndarray, enhanced: np.ndarray, segmentation: Dict[str, Any], out_path: Path, filename: str) -> Optional[str]:
        if segmentation.get("method") != "boundary_continuation_test2":
            return None
        original_u8 = self._normalize_to_uint8(original)
        ...
        for ax, img, title in [
            (axes[0], original_u8, "Original + Test 2 Paths"),
            (axes[1], enhanced, "Enhanced + Filled Envelope"),
        ]:
            ax.imshow(img, cmap="gray")
            if upper is not None:
                ax.plot(upper[:,0], upper[:,1], linewidth=1.5, label="upper outer path")
            if lower is not None:
                ax.plot(lower[:,0], lower[:,1], linewidth=1.5, label="lower outer path")
            pts = contour.reshape(-1,2)
            ax.plot(np.r_[pts[:,0], pts[0,0]], np.r_[pts[:,1], pts[0,1]], linewidth=1.0, alpha=0.8)
            ax.set_title(title)
    ...
    def process_file(self, filepath: Path, output_dir: Optional[Path] = None) -> Dict[str, Any]:
        ...
        if self.config.save_png:
            try:
                self._save_boundary_test2_png(bf, processed["enhanced"], segmentation, boundary_test2_path, filepath.name)
            except Exception as exc:
